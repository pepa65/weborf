...
#ifdef IPV6
        char *suggestion="If the kernel has no IPv6 support, recompile weborf without IPV6 defined in options.h.in\n";
        ssize_t retval = write(2, suggestion, strlen(suggestion));
#endif
        return -1;
    }
...
void net_bind_and_listen(int s) {
    ...
    int ipAddrL=sizeof(struct sockaddr_in6);
#else
