...
// This function is designed to be similar to a normal read, but it uses an
// internal buffer. When the buffer is empty, it will try to fill it.  An 
// important difference from the normal read is that this function will wait
// until the requested amount of bytes are available, or until the timeout
// occurs. Timeout duration is defined with the READ_TIMEOUT define. On some
...
// This function returns how many bytes must be read in order to read enough
// data for it to end with the string needle. strlen(needle) must be added to
// the returned value to include the needle string in the result of the read.
// If the string needle is not in the buffer then the entire size of the data
// present in cache will be return.
