...
// This function will create a copy of the source URI into the dest buffer
// and %-escaping it. dest_size specifies the size of the dest buffer
...
// This function will use the result param prop to return the required props
// and the value of the Depth header. Properties are in the form
// <D:prop><prop1/><prop2/></D:prop>
// If something unexpected happens during the xml parsing, ERR_NODATA
// will be returned. RFC-1518 requires that the xml must be validated and an
// error MUST be returned to the client if the xml is not valid. This is not
// the case in this funcion. It will accept many forms of invalid xml.
...
static inline int get_props(connection_t* connection_prop,string_t* post_param,u_dav_details *props) {

    {
        ...
        ///props.deep=false; // commented because redundant
    ...
    if (post_param->len==0) { // No specific prop request, sending everything
        ...
        ///props->getcontenttype=false; // Commented because redundant
...
// This function sends a xml property to the client. It can be called only by
// funcions aware of this xml, because it sends only partial xml.
...
// This function serves a PROPFIND request. Can serve both depth and non-depth
// requests. This funcion works only if authentication is enabled.
int propfind(connection_t* connection_prop,string_t *post_param) {
    ...
    {
        // This redirects directory without final / to directory with final /
    ...
    // xml) and sends a multistatus header code
    ...
    if (props.dav_details.deep) { // Send children files
        ...
        char file[URI_LEN];
        struct dirent *entry;
        if (dp==NULL) { // Error, unable to send because header already sent
            close(sock);
            return 0;
        }

        // Cycles through dir's elements
        while ((entry=readdir(dp)) != NULL) {
#ifdef HIDE_HIDDEN_FILES
            if (entry->d_name[0]=='.') continue; // doesn't list hidden files
#else
    ...
    // sent to the client, so now we read from cache and send to the client
