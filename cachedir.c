...
// Returns true if the caching is enabled and false otherwise
...
// Sends a cached item if present and returns true. Returns false on cache miss.
...
// item. If a cache miss occurs -1 will be returned
...
// Acquires a shared lock on the file, so if the file is already opened in
// write mode the lock will fail and the function will return the same result
// of a cache miss.
...
// Stores the content of the buffer "content" in cache, for the size
// specified by content_len. The cache file will have an exclusive lock to
// prevent multiple threads accessing the same file in write mode.
// Lock is acquired in a non-blocking way, so if the file is already locked
// the method will just return without writing anything on the file. This
// behavior is wanted because it is assumed that the lock is held by another
// thread writing the same content on the file, waiting for the lock to be
// released would lead to override the content of the file with the same
// content.
void cache_store_item(unsigned int uprefix,connection_t* connection_prop, char *content, size_t content_len) {
    ...
    // Try to acquire lock in a non-blocking way, and exit if another thread
    // has that lock
    if (flock(fd,LOCK_EX|LOCK_NB)!=0) {
        close(fd);
        return;
...
// This function initializes a cache directory. If the dir is not a directory
// or it is impossible to stat, weborf will terminate. If it is impossible to
// delete and create files in it, weborf will just log a warning.
