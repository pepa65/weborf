...
static inline void set_connection_props(connection_t *connection_prop) {
    ...
    // Obtains the connection header, writing it into the a buffer, and sets
    // connection=true if the header is present
...
static inline void handle_requests(char* buf,buffered_read_t * read_b,int * bufFull,connection_t* connection_prop,long int id) {
    ...
    while (true) { // Infinite cycle to handle all pipelined requests
        if ((*bufFull)!=0) {
            // Sets to 0 the buffer, only the part used for the previous
            // request in the same connection
            memset(buf,0,(*bufFull));
        ...
        while ((end=strstr(buf+from,"\r\n\r"))==NULL) {
            ...
            if (!(buf[*bufFull]==10 || buf[*bufFull]==13)) {
                // Optimization to make strstr parse only the ending part of
                // the string
                from=(*bufFull);
            }
        ...
        if (strstr(buf+from,"\r\n\r\n")==NULL) {
            // If we didn't read yet the lst \n of the ending sequence, we
            //  read it now, so it won't disturb the next request
            // Reads 1 char and adds to the buffer
...
// Function executed at the beginning of the thread. Takes open sockets from
// the queue and serves the requests. Doesn't do busy waiting.
void * instance(void * nulla) {
    thread_prop_t thread_prop;  // Server's props
    // Set thread_prop as thread variable
    pthread_setspecific(thread_key, (void *)&thread_prop);
    ...
    connection_t connection_prop; // Struct with connection properties
    buffered_read_t read_b; // Buffer for buffered reader
    int sock=0; // Socket with the client
    char * buf=calloc(INBUFFER+1,sizeof(char)); // Buffer for HTTP request
...
// It requires the socket, connection_t struct and buffered_read_t to read
// from the socket, since the reading is internally buffered.
// PUT size has no hardcoded limits. Auth provider has to check for the file's
// size and refuse it if it is the case. This function will not work if there
// is no auth provider.
int read_file(connection_t* connection_prop,buffered_read_t* read_b) {
    ...
    // If unsupported Content-* header, then return 501 (Not implemented)
    {
    ...
    if (file_exists(connection_prop->strfile)) {
        // Resource already existed (No content)
        retval=OK_NOCONTENT;
...
// list of files within the specified directory.
int write_dir(char* real_basedir,connection_t* connection_prop) {
    int sock=connection_prop->sock;

    // WARNING: This code checks the ETag and returns if the client has a copy
    // in cache since the impact of using ETag for generated directory list is
    // not known yet, if ETag goes away, also the following block will have to
    // be deleted
    {
...
// Writes a file to the socket, compressing it with gzip. Then, since it is
// not possible to know the size of the compressed file, it is not possible to
// send the size in advance, so it will set keep_alive to false
...
// finds out the mimetype, sends the header.
...
static inline unsigned long long int bytes_to_send(connection_t* connection_prop,char *a) {
    ...
    if (range_header && connection_prop->strfile_stat.st_mtime==etag) {
        // Find if it is a range request 5 is strlen of "range"
...

        {
            ...
            if ((eq=strstr(a,"="))==NULL ||(sep=strstr(eq,"-"))==NULL) {
...
// This function reads a file and writes it to the socket. Also sends the
// http header with the content length header same as the file's size.
// There is no limit to file size, and it uses O_LARGEFILE flag to send file
// larger than 4gb. Since this flag isn't available on all systems, it will
// be 0 when compiled on system who doesn't have this flag. On those systems
// it is unpredictable if it will be able to send large files or not.
// If the file is larger, it will be sent using write_compressed_file,
// see that function for details.
// To work connection_prop.strfile_fd and connection_prop.strfile_stat must be
// set. The file sent is the one specified by strfile_fd, and it will not be
//  closed after.
int write_file(connection_t* connection_prop) {

    int sock=connection_prop->sock;

    // Buffer for Range, Content-Range headers, and reading if-none-match from header
    char a[RBUFFER+MIMETYPELEN+16];
...
// This function reads post data and returns the pointer to the buffer
// containing the data (if there is any) or NULL if there was no data.
...
// This function returns a pointer to a string. It will use virtualhost
// settings to return this string and if no virtualhost is set for that host,
// it will return the normal basedir.
...
// This function returns the reason phrase according to the response code
...
// This function sends a code header to the specified socket, size is the
// Content-Length field, headers can be NULL or some extra headers to add.
...
// This function will automatically take care of generating Connection header
// when needed, according to keep_alive and protocol_version of connection_prop
...
// This function writes on the specified socket a tar.gz file containing the
// required directory. No caching at all is used or is allowed to the clients
