...
// Copies count bytes from file descriptor "from" to the file descriptor "to".
// It is possible to use lseek on the descriptors before calling this function.
...
// Deletes a directory and its content. This function is something like rm -rf
// dir is the directory to delete
// file is a buffer. Allocated outside because it will be reused by every
// recursive call. Its size is file_s. Returns 0 on success
int dir_remove(char * dir) {
    // If it is a file, removes it, otherwise list the directory's content,
    // then do a recoursive call and do rmdir on self
    if (unlink(dir)==0)
        return 0;
...
#ifdef WEBDAV
// Moves a file. If it is on the same partition it will create a new link and
// delete the previous link. Otherwise it will create a new copy and delete
...
// Copies a file into another file. Returns 0 on success
...
// This function copies a directory. The destination directory will be created
// and will be filled with the same content of the source directory
// Returns 0 on success
int dir_copy (char* source, char* dest) {
    return dir_move_copy(source,dest,COPY);
}

// This function moves a directory. The destination directory will be created
// and will be filled with the same content of the source directory.
// Then, the source directory will be deleted. Returns 0 on success
...
int dir_move_copy (char* source, char* dest,int method) {
    ...
    if (mkdir(dest,S_IRWXU | S_IRWXG | S_IRWXO)!=0) {
        // Attemps to create destination directory
        return ERR_FORBIDDEN;
    }

    DIR *dp=opendir(source); // Open dir
    struct dirent *entry;
    if (dp==NULL) return ERR_FILENOTFOUND;
